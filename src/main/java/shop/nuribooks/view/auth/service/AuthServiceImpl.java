...
//        if (Boolean.TRUE.equals(redisTemplate.hasKey(request.username()))) {
//            throw new BadRequestException("인증 번호가 이미 발송되었습니다. 메시지를 확인하세요.");
//        }

        redisTemplate.opsForValue().set(request.username(), String.valueOf(checkNum), 3, TimeUnit.MINUTES);

        return messageSender.sendMessage(
                request, new MessageRequest("휴면 인증", "휴면회원 인증번호입니다: " + checkNum));
    }

    @Override
    public void reactiveMember(MemberReactiveRequest request) {

        String restoredAuthenticationCode = redisTemplate.opsForValue().get(request.username());

        if (!request.authenticationCode().equals(restoredAuthenticationCode)) {
            // 인증번호가 일치하지 않음
